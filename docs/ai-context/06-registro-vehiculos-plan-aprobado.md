...
1. Correr `./run.ps1 reset-db` contra una base real y confirmar que las tablas nuevas se crean sin error (los scripts nunca se ejecutaron contra SQL Server, solo se revisaron por inspección).
2. Levantar todo con `./run.ps1 up` y probar el flujo real: crear compañía → invitar → aceptar → cambiar contexto → registrar vehículo → alcanzar cuota → verificar 403 en accesos cruzados. Ver la sección "Verificación end-to-end" del plan completo (13 puntos) para el checklist exacto.
